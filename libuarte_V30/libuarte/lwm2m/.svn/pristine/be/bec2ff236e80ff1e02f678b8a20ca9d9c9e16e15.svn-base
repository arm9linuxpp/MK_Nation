...
#ifndef NDEBUG
static int maxlog = DTLS_LOG_DEBUG; // DTLS_LOG_WARN;      /* default maximum log level */ // !!!MZA???
...
/*
  ...
  FILE* log_fd = level <= DTLS_LOG_CRIT ? stdxerr : stdxout;
...
void hexdump(const unsigned char *packet, int length) {
  ...
  while (length--) {
    if (n % 16 == 0)
      printf("%08X ",n);         // DO not use on embedded device!!!
