...
// tcpdump -n udp port 7685
    g_LwM2M.lwm2mH = NULL;
    g_LwM2M.server = "168.119.70.243";  // VI Server.
    ...
    #ifdef MU_DTLS
    g_LwM2M.name = "urn:imei:864486061884090";  // same as COT
    #else
            ...
            if (FD_ISSET(data.sock, &readfds))
            {
                ...
                else if (0 < numBytes)
                {
                    ...
                    if (connP != NULL)
                    {
                        ...
                        {
                            char msg[255];
                            sprintf(msg,"error handling message %d\n",result);
                            viFailureLog(msg);
                        }
#else
...
xxx = 42;  // debug poin
    ...
    {
        char msg[255];  
        sprintf(msg,"lwm2m_connection_handle_packet Error 0x%08x(%d) ",result,result);
        viFailureLog(msg);
    } else {
