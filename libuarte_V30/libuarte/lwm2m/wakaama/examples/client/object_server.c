...
static void remove_optional_resource(uint16_t *resList, int val, const uint_fast16_t id, size_t *nbRes) {
    if (val >= 0) {
...
static void remove_all_optional_resources(uint16_t *resList, server_instance_t *targetP, size_t *nbRes) {
                               ...
                               lwm2m_object_t * objectP)
{
    ...
    if (*numDataP == 0)
    {
        ...
        size_t nbRes = sizeof(resList) / sizeof(uint16_t);
...
{
    char msg[255];
    sprintf(msg, "  /%u: Server object, instances:\r\n", object->objID);
    viDebugLog(msg);
    ...
    {
        char msg[255];
        sprintf(msg, "    /%u/%u: instanceId: %u, shortServerId: %u, lifetime: %u, storing: %s, binding: %s",
        ...
        {
            sprintf(msg, ", registrationPriorityOrder: %d", serverInstance->registrationPriorityOrder);
            viDebugLog(msg);
        }
        if(serverInstance->initialRegistrationDelayTimer >= 0)
        {
            sprintf(msg, ", initialRegistrationDelayTimer: %d", serverInstance->initialRegistrationDelayTimer);
            viDebugLog(msg);
        }
        if(serverInstance->registrationFailureBlock >= 0)
        {
            sprintf(msg, ", registrationFailureBlock: %s",
                    serverInstance->registrationFailureBlock > 0 ? "true" : "false");
            viDebugLog(msg);
        }
        if(serverInstance->bootstrapOnRegistrationFailure >= 0)
        {
            sprintf(msg, ", bootstrapOnRegistrationFaulure: %s",
                    serverInstance->bootstrapOnRegistrationFailure > 0 ? "true" : "false");
            viDebugLog(msg);
        }
        if(serverInstance->communicationRetryCount >= 0) 
        {
            sprintf(msg, ", communicationRetryCount: %d", serverInstance->communicationRetryCount);
            viDebugLog(msg);
        }
        if(serverInstance->communicationRetryTimer >= 0)
        {
            sprintf(msg, ", communicationRetryTimer: %d", serverInstance->communicationRetryTimer);
            viDebugLog(msg);
        }
        if(serverInstance->communicationSequenceDelayTimer >= 0)
        {
            sprintf(msg, ", communicationSequenceDelayTimer: %d", serverInstance->communicationSequenceDelayTimer);
            viDebugLog(msg);
        }
        if(serverInstance->communicationSequenceRetryCount >= 0)
        {
            sprintf(msg, ", communicationSequenceRetryCount: %d", serverInstance->communicationSequenceRetryCount);
            viDebugLog(msg);
        }
        sprintf(msg, ", muteSend: %s", serverInstance->muteSend ? "true" : "false");
        viDebugLog(msg);
#endif
        //fprintf(stdxout, "\r\n");
